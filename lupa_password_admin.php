...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($query && mysqli_num_rows($query) > 0) {
        ...
        $resetLink = "http://localhost/ProjectPengaduanMasyarakat/reset_password_admin.php?token=$token";
        date_default_timezone_set('Asia/Jakarta');
        $expires = date("Y-m-d H:i:s", strtotime("+1 hour"));
        $update = mysqli_query($conn, "UPDATE petugas SET reset_token='$token', token_expiry='$expires' WHERE email='$email'");
        ...
        if ($update) {
            ...
            try {
                ...
                $mail->SMTPAuth   = true;
                $mail->Username   = 'ff0174ef07c2f1';
                $mail->Password   = '3a867d23e89f3e';
                ...
                $template = file_get_contents('config/email_lupa_password_admin.html');
